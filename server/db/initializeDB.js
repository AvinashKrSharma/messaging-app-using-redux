...
// template returns a function that expects an object with keys same as the ones used in template string.
// change.pick return one random element from array
...
export const getRandomMessage = (userIDs)=>({
    // chance.guid return a version 5 guid
    id:chance.guid(),
    owner: chance.pick(userIDs),
    content: {
        text: getRandomMessageText()
    },
    // chance.date returns a random date

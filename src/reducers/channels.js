import { UPDATE_CHANNEL_INPUT_TEXT, UPDATE_CHANNEL_FETCHED_STATUS, SET_CHANNEL_INFO, SUBMIT_CHANNEL_INPUT_TEXT, RECEIVE_MESSAGE, REQUEST_CREATE_CHANNEL, COMPLETE_CHANNEL_CREATION, FETCHED } from './../actions/';
import { createReducer } from './../utility';
import { fromJS } from 'immutable';

const receiveMessageHandler = (state, { message: { id, channelID, content, owner } }) => {
    const index = state.findIndex(
      (channel) => { return channel.get(`id`) === channelID });
    if (index === -1) {
      return state;
    }
